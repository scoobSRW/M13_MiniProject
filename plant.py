...
from app.models.models import (Customer, Employee, Order, Product, Production,
                               Role, User, db)
        ...
        with app.app_context():
            ...
            for emp_data in employees:
                if not Employee.query.filter_by(name=emp_data["name"]).first():
                    employee = Employee(
                        name=emp_data["name"], position=emp_data["position"]
                    )
            ...
            productions = [
                {
                    ...
                    "date_produced": date(2023, 12, 25),
                },
                {
                    "product_name": "Gadget",
                    "quantity_produced": 200,
                    "date_produced": date(2023, 12, 26),
                },
            ]

            for prod_rec_data in productions:
                product = Product.query.filter_by(
                    name=prod_rec_data["product_name"]
                ).first()
                if (
                    product
                    and not Production.query.filter_by(
